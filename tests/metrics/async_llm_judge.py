...
class AsyncLLMJudgeMetric(MetricBase):
    ...
    def calculate(self, answer: str, expected: str, keywords: Optional[List[str]] = None, question: Optional[str] = None) -> float:
        ...
        Args:
            ...
            keywords: Not used
            question: The actual question asked by the user
        ...
        if _executor:
            _executor.submit(_grade_one, question, expected, answer)
...
def _perform_attempt(question: str, expected: str, answer: str) -> Dict:
    ...
    prompt = _build_grading_prompt(question, expected, answer)
...
def _grade_one(question: str, expected: str, answer: str):
    ...
    for attempt in range(max_retries):
        try:
            result = _perform_attempt(question, expected, answer)
            with _results_lock:
...
def _build_grading_prompt(question: str, expected: str, answer: str) -> str:
...
1. **Accuracy (40%)**: Are the facts, concepts, and technical details correct according to the expected ground truth answer?
2. **Completeness (30%)**: Does the answer fully address all aspects of the question as established by the ground truth?
...
**Instructions:**
- Base your evaluation ONLY on the expected ground truth answer
